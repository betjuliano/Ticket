...
interface DashboardData {
  ...
  charts: {
    ...
    ticketsByPriority: Array<{ priority: string; count: number; label: string }>
    ticketsByCategory: Array<{ category: string; count: number; label: string }>
    ticketsOverTime: Array<{ date: string; count: number }>
    topCategories: Array<{ category: string; count: number; label: string }>
    priorityDistribution: Array<{ priority: string; count: number; label: string; color: string }>
  }
  recentTickets: Array<{
    id: string
    title: string
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold">Dashboard Analítico</h1>
          <p className="text-gray-600">Visão geral dos chamados e estatísticas</p>
        </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{data.summary.open}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-yellow-600">{data.summary.inProgress}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{formatHours(data.summary.avgResolutionTime)}</p>
              </div>
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        <TabsList>
          ...
          {userRole !== 'USER' && <TabsTrigger value="users">Usuários</TabsTrigger>}
        </TabsList>
                      ...
                      <div>
                        ...
                        <Badge variant="secondary" className="mt-1">
                          {user.role === 'ADMIN' ? 'Admin' : 
                           user.role === 'COORDINATOR' ? 'Coordenador' : 'Usuário'}
                        </Badge>
                      </div>
                      <div className="text-right space-y-1">
                        <div className="text-sm">
                          <span className="text-gray-600">Criados:</span> {user.createdCount}
                        </div>
                        <div className="text-sm">
                          <span className="text-gray-600">Atribuídos:</span> {user.assignedCount}
                        </div>
                        <div className="text-sm">
                          <span className="text-gray-600">Resolvidos:</span> {user.resolvedCount}
                        </div>
                      </div>
                    </div>
                  ))}
