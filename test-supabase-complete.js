...
async function testSupabaseSetup() {
  ...
  const optionalEnvs = ['SUPABASE_SERVICE_ROLE_KEY', 'SUPABASE_JWT_SECRET'];

  for (const env of optionalEnvs) {
    if (
      process.env[env] &&
      process.env[env] !== '[YOUR_SERVICE_ROLE_KEY]' &&
      process.env[env] !== '[YOUR_JWT_SECRET]'
    ) {
      console.log(`✅ ${env}: Configurada`);
    } else {
  ...
  try {
    ...
    const subscriptionPromise = new Promise(resolve => {
      const timeout = setTimeout(() => {
        console.log(
          '⚠️  Real-time: Timeout (pode estar funcionando, mas lento)'
        );
        resolve(false);
      }, 5000);
