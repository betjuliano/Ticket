...
class Logger {
  ...
  apiRequest(method: string, url: string, userId?: string, ip?: string, userAgent?: string): void {
  ...
  cacheOperation(operation: string, key: string, hit: boolean, duration?: number): void {
  ...
  rotateLogs(): void {
    try {
      ...
      this.error('Log rotation failed', { error: error instanceof Error ? error.message : String(error) })
    }
  }
}
