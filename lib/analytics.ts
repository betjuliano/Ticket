    ...
    return cache.getOrSet(cacheKey, async () => {
      try {
        ...
        const [lowPriority, mediumPriority, highPriority, urgentPriority] = await Promise.all([
          prisma.ticket.count({ where: { ...dateFilter, priority: 'LOW' } }),
          prisma.ticket.count({ where: { ...dateFilter, priority: 'MEDIUM' } }),
          prisma.ticket.count({ where: { ...dateFilter, priority: 'HIGH' } }),
          prisma.ticket.count({ where: { ...dateFilter, priority: 'URGENT' } })
        ])
        ...
        const topResolversFormatted = topResolvers.map(user => {
          ...
          const avgResolutionTime = resolvedCount > 0
            ? user.assignedTickets.reduce((acc, ticket) => {
                const diff = new Date(ticket.closedAt!).getTime() - new Date(ticket.createdAt).getTime()
                return acc + diff
              }, 0) / resolvedCount / (1000 * 60 * 60) // em horas
            : 0
    ...
    return cache.getOrSet(cacheKey, async () => {
      try {
        ...
        const firstResponseTimes = ticketsWithComments
          ...
          .map(ticket => {
            const diff = new Date(ticket.comments[0].createdAt).getTime() - new Date(ticket.createdAt).getTime()
  ...
  async getInsights(period: string = '30d'): Promise<any> {
    ...
    return cache.getOrSet(cacheKey, async () => {
      try {
        ...
        const highPriorityRatio = (ticketStats.byPriority.high + ticketStats.byPriority.urgent) / ticketStats.total
