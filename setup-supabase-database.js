...
async function setupSupabaseDatabase() {
    ...
    async function executeSQLScript(scriptName, sqlContent) {
        ...
        try {
            ...
            if (error) {
                console.log(`⚠️  RPC não disponível para ${scriptName}:`, error.message);
                return false;
            } else {
    ...
    try {
        ...
        if (error && error.message.includes('Could not find the function')) {
            ...
            console.log('📋 Você precisará executar os scripts manualmente no painel do Supabase');
    ...
    try {
        ...
        if (listError) {
            ...
            if (!ticketBucket) {
                ...
                const { data: bucket, error: createError } = await supabase.storage.createBucket('ticket-attachments', {
                    public: false,
                    allowedMimeTypes: [
                        'image/jpeg',
                        'image/png',
    ...
    console.log('1. Execute os scripts SQL no painel do Supabase (se não executados automaticamente)');
