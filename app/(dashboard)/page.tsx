      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-400">
            Acompanhe seus chamados e abra novos tickets
          </p>
        </div>
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const activeUsers =
          usersData?.users?.filter((user: any) => user.isActive)?.length || 0;

        // Calcular estatísticas de tickets - ajustar para o formato da API
        const tickets = ticketsData?.data || [];
        const totalTickets = tickets.length || 0;
        const openTickets =
          tickets.filter((ticket: any) => ticket.status === 'open')?.length ||
          0;
        const inProgressTickets =
          tickets.filter((ticket: any) => ticket.status === 'in_progress')
            ?.length || 0;
