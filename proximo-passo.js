...
async function proximosPasso() {
    ...
    if (abrirSupabase.toLowerCase() === 's' || abrirSupabase.toLowerCase() === 'sim') {
        console.log('🌐 Abrindo painel do Supabase...');
        exec('start https://supabase.com/dashboard', (error) => {
            if (error) {
                console.log('⚠️  Não foi possível abrir automaticamente. Acesse: https://supabase.com/dashboard');
            }
        });
    ...
    try {
        ...
        const hasJwtSecret = envContent.includes('SUPABASE_JWT_SECRET=') && 
                            !envContent.includes('[YOUR_JWT_SECRET]');
        const hasDatabaseUrl = envContent.includes('DATABASE_URL=') && 
                              envContent.includes('postgresql://');
        
        console.log(`   ${hasServiceRole ? '✅' : '⚠️ '} SUPABASE_SERVICE_ROLE_KEY`);
    ...
    if (executarTeste.toLowerCase() === 's' || executarTeste.toLowerCase() === 'sim') {
        console.log('\n🧪 Executando teste...');
